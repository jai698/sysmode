  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.trollgold.org/persistventures/assignment/mark_eval_internshala?${queryParams}`,
        {
          method: 'PATCH',
          headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to mark evaluation');
      }

      setSuccess(true);
      message.success({
        content: <span style={{ color: '#000000', fontWeight: '600' }}>Evaluation marked successfully! 🎉</span>,
        style: {
          position: 'fixed',
          left: '50%',
      ...
      message.error({
        content: <span style={{ color: '#000000', fontWeight: '600' }}>Failed to mark evaluation: {error.message}</span>,
        style: {
          position: 'fixed',
          left: '50%',
          transform: 'translateX(-50%)',
    ...
    <div className="h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 p-4">
      <div className="max-w-3xl mx-auto h-full" data-aos="fade-up">
        ...
        <div className="flex justify-between items-center h-[50px]">
          <div>
            <h1 className="text-2xl font-bold text-white">Mark Evaluation</h1>
          </div>
          <Button 
        ...
        <div className="bg-gray-800 rounded-xl shadow-xl border border-gray-700/50 p-4 mt-2">
          <Alert
            message="Evaluation Marking System"
            description="Please ensure all information is correct before submitting. This action cannot be undone."
            type="info"
            showIcon
            className="mb-4"
          />
          ...
          >
            ...
            <Button 
              ...
              disabled={loading}
            >
              {loading ? 'Marking Evaluation...' : 'Mark Evaluation'}
            </Button>
            ...
            <div className="mt-3 bg-gray-700/30 p-3 rounded-xl border border-gray-600">
              ...
              <ul className="list-disc list-inside text-gray-300 space-y-1 text-sm">
                ...
                <li>This action marks the evaluation as complete</li>
                <li>The action cannot be reversed</li>
                <li>System will update the candidate's status automatically</li>
              </ul>
            </div>
            ...
            <div className="p-2 bg-gray-700/30 rounded-lg border border-gray-600">
              <div className="flex items-center text-sm text-gray-300">
                ...
                <span>All evaluation marks are logged for tracking purposes.</span>
              </div>
            </div>
          </Form>
          ...
          {success && (
            <Alert
              message="Evaluation Marked Successfully"
              type="success"
              showIcon
              className="mt-3"
            />
      ...
      {loading && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-lg shadow-xl text-center border border-gray-700">
            ...
            <p className="text-gray-300">Marking Evaluation...</p>
          </div>
        </div>
      )}
    </div>
